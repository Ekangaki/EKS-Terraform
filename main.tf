...
resource "aws_subnet" "afro-it_subnet" {
  ...
  tags = {
    Name = "afro-it-subnet-${count.index}"
  }
}
...
resource "aws_route_table" "afro-it_route_table" {
  ...
  route {
    ...
    gateway_id = aws_internet_gateway.afro-it_igw.id
  }

  tags = {
    Name = "afro-it-route-table"
  }
}

resource "aws_route_table_association" "a" {
  count          = 2
  subnet_id      = aws_subnet.afro-it_subnet[count.index].id
...
resource "aws_security_group" "afro-it_cluster_sg" {
  ...
  tags = {
    Name = "afro-it-cluster-sg"
  }
}

resource "aws_security_group" "afro-it_node_sg" {
  vpc_id = aws_vpc.afro-it_vpc.id
...
resource "aws_iam_role_policy_attachment" "afro-it_cluster_role_policy" {
  role       = aws_iam_role.afro-it_cluster_role.name
...
resource "aws_iam_role_policy_attachment" "afro-it_node_group_cni_policy" {
  role       = aws_iam_role.afro-it_node_group_role.name
  policy_arn = "arn:aws:iam::aws:policy/AmazonEKS_CNI_Policy"
}

resource "aws_iam_role_policy_attachment" "afro-it_node_group_registry_policy" {
  role       = aws_iam_role.afro-it_node_group_role.name
